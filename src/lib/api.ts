...
const EXERCISES_BASE_URL = "https://cdn.jsdelivr.net/gh/wrkout/exercises.json@master/exercises";
...
"Hug_Knees_To_Chest",
"Hurdle_Hops",
"Hyperextensions_(Back_Extensions)",
...
"Kettlebell_Thruster",
"Kettlebell_Turkish_Get-Up_(Lunge_style)",
"Kettlebell_Turkish_Get-Up_(Squat_style)",
"Kettlebell_Windmill",
"Kipping_Muscle_Up",
...
"Machine_Shoulder_(Military)_Press",
...
"Pushups",
"Pushups_(Close_and_Wide_Hand_Positions)",
"Pyramid",
"Quad_Stretch",
"Quadriceps-SMR",
"Quick_Leap",
...
"See-Saw_Press_(Alternating_Side_Press)",
"Shotgun_Row",
"Shoulder_Circles",
...
export const generateExerciseImages = (exerciseName: string): string[] => {
  return [
    `${EXERCISES_BASE_URL}/${exerciseName}/images/0.jpg`,
    `${EXERCISES_BASE_URL}/${exerciseName}/images/1.jpg`
  ];
};

export const fetchExerciseData = async (exerciseName: string): Promise<Exercise> => {
  const response = await fetch(`${EXERCISES_BASE_URL}/${exerciseName}/exercise.json`);
  if (!response.ok) {
    throw new Error(`Failed to fetch exercise data for ${exerciseName}`);
  }
  const data = await response.json();
  
  return {
    ...data,
    images: generateExerciseImages(exerciseName)
  };
};